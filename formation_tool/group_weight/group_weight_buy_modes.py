...
def append_buy_like_group_rows_with_optional_inference(
    rows,
    mode,
    source_rebates,
    rules,
    ...
    if should_infer:
        mode_rows, infos = group_weight_row_helpers.append_targeted_buy_like_group_weight_rows(
            rows,
            write_game_type,
            pairs,
            multiplier,
        )
        first_info = infos.get(int(WEIGHT_GROUP_IDS[0])) if WEIGHT_GROUP_IDS else None
        return {
            'write_game_type': int(write_game_type),
            'multiplier': float(multiplier),
            'pairs': pairs,
            ...
            'should_infer': should_infer,
            'mode_rows': mode_rows,
            'rtp': None if first_info is None else first_info.get('actual_rtp'),
            'display_rtp': None if first_info is None else first_info.get('display_rtp'),
            'infos': infos,
        }
...
def append_extra_buy_group_weight_rows(rows, source_rebates, extra_buy):
    ...
    return append_buy_like_group_rows_with_optional_inference(
        rows,
        make_extra_buy_mode(write_game_type),
        source_rebates,
...
def append_buy_group_weight_modes(rows, rebates_by_mode, mode_exists, mode_pairs):
    ...
    if BUY_GROUP_ENABLED:
        ...
        if not mode_exists.get(game_type, False):
            ...
            if buy_info['should_infer']:
                print(
                    f"\n[{mode_name}] rebate=0 反推开启，game_type={write_game_type}，"
                    f"购买倍数={format_weighted_rtp(BUY_GROUP_MULTIPLIER)}，"
                    f"准备写入 {buy_info['mode_rows']} 行"
                )
            else:
                print(
                    f"\n[{mode_name}] RTP={format_weighted_rtp(buy_info['rtp'])}，game_type={write_game_type}，"
                    f"购买倍数={format_weighted_rtp(BUY_GROUP_MULTIPLIER)}，"
                    f"显示RTP={format_weighted_rtp(buy_info['display_rtp'])}，准备写入 {buy_info['mode_rows']} 行"
                )
    ...
    for extra_buy in EXTRA_BUY_GROUPS:
        ...
        if extra_info['should_infer']:
            print(
                f"\n[额外购买局 game_type={write_game_type}] rebate=0 反推开启，"
                f"购买倍数={format_weighted_rtp(extra_info['multiplier'])}，"
                f"跳过 weight=0 的 rebate {extra_info['skipped_zero']} 个，"
                f"准备写入 {extra_info['mode_rows']} 行"
            )
        else:
            print(
