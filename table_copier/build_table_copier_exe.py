"""Build encrypted table_copier.exe for the table copier tool."""
import subprocess
import sys
...
MAIN_PATH = TOOL_ROOT / "table_copier.py"
CONFIG_PATH = TOOL_ROOT / "config.py"
DB_CONFIG_PATH = PROJECT_ROOT / "db_config.py"
LAUNCHER_PATH = BUILD_ROOT / "table_copier_encrypted_launcher.py"
SPEC_PATH = BUILD_ROOT / "table_copier.spec"
EXE_PATH = DIST_ROOT / "table_copier.exe"
...
def build_launcher():
    if not MAIN_PATH.is_file():
        raise FileNotFoundError(f"未找到表复制主脚本: {MAIN_PATH}")
    if not CONFIG_PATH.is_file():
        raise FileNotFoundError(f"未找到表复制配置文件: {CONFIG_PATH}")
    ...
    config_payload = encrypt_text(fernet, CONFIG_PATH)
...
_CONFIG_PAYLOAD = {config_payload!r}
...
def _install_module(name, payload, filename):
    source = _decrypt(payload)
...
def _run_main(app_dir):
    ...
    filename = os.path.join(app_dir, 'table_copier.py')
    globals_dict = {{
        '__name__': '__main__',
        '__file__': filename,
        '__package__': None,
...
def main():
    ...
    tool_dir = os.path.join(app_dir, 'table_copier')
    if app_dir not in sys.path:
        sys.path.insert(0, app_dir)
    if tool_dir not in sys.path:
        sys.path.insert(0, tool_dir)

    _install_module('db_config', _DB_CONFIG_PAYLOAD, os.path.join(app_dir, 'db_config.py'))
    _install_module('config', _CONFIG_PAYLOAD, os.path.join(tool_dir, 'config.py'))
    _run_main(tool_dir)
...
a = Analysis(
    [str(BUILD_ROOT / "table_copier_encrypted_launcher.py")],
